...
void choose_grid(FlowField flow) {
  ...
  for (float i = head - PI/3; i < head + PI/3; i += 0.15) {
     ...
     PVector sightline = PVector.add(tmp_loc, new PVector(60 * flow.resolution * cos(i), 60 * flow.resolution * sin(i)));
...
boolean has_arrived(PVector destination) {
    ...
    return loc.dist(destination) < 100;
